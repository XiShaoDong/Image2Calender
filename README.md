...
| 角色 | 凭据 | 权限 |
|---|---|---|
| 访客 | 访问码（ACCESS_CODE） | 仅使用 OCR 和生成日历，**不能**改任何设置 |
| 管理员 | 管理员密码（ADMIN_PASSWORD） | 改模式、改 key、改访问码、查看 key |

- **私密（默认）**：访客需输入访问码才能用；管理员在设置区解锁后可切换为**公开**（所有人免码可用）
- 云端部署时全部用环境变量（文件系统不可持久化）：
  - `OCRSPACE_KEY`（必填）
  - `GEMINI_KEY`（选填）
  - `ADMIN_PASSWORD`（强烈建议，否则任何知道访问码的人都能改设置）
  - `ACCESS_CODE` = 访客访问码
  - `PUBLIC_MODE=1` = 公开模式（所有人都能用，且共用你的免费 API 额度）
- 忘记访问码/管理员密码：本地删除 config.json 中对应字段；云端改环境变量
